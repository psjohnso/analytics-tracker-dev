...
// ─── CONTRIBUTORS (task-level) ────────────────────────────────────────
// Tasks have an assignee (primary owner) AND optionally a contributors
// list (comma-separated names, mirrors projects' other_members field).
// One canonical helper set so the form, detail view, list rows, and My
// Work filter all read contributors the same way.

function getTaskContributors(t) {
  if (!t || !t.contributors) return [];
  return String(t.contributors).split(',')
    .map(function(s) { return s.trim(); })
    .filter(Boolean);
}

function isTaskContributor(t, name) {
  if (!name) return false;
  var list = getTaskContributors(t);
...
// True when the named person is either the assignee OR a contributor.
// Used by My Work to include "tasks I'm helping with" in the same view
// as "tasks I own."
function isTaskMineOrContributing(t, name) {
  if (!name || !t) return false;
  if (t.assignee === name) return true;
  return isTaskContributor(t, name);
}

// Render a small avatar stack of a task's contributors for compact list
// rows. Returns '' when there are no contributors (so the call site can
// concat unconditionally without padding showing up). Max `cap` avatars,
// then "+N" if there are more.
function renderContributorAvatars(t, opts) {
  var contribs = getTaskContributors(t);
  if (contribs.length === 0) return '';
  opts = opts || {};
  var size = opts.size || 18;
  var cap  = opts.max  || 3;
  var shown = contribs.slice(0, cap);
  var rest  = contribs.length - shown.length;
  var avFontSize = Math.max(8, Math.round(size * 0.45));
  var html = '<span class="contrib-stack" title="Contributors: ' +
    contribs.map(function(n) { return n.replace(/"/g, '&quot;'); }).join(', ') +
    '" style="display:inline-flex;align-items:center;margin-left:6px;vertical-align:middle;">';
  shown.forEach(function(name, i) {
    var init = name.split(/\s+/).map(function(w) { return w[0] || ''; }).join('').slice(0, 2).toUpperCase();
    html += '<span class="contrib-av" style="display:inline-flex;align-items:center;justify-content:center;width:' + size + 'px;height:' + size + 'px;border-radius:50%;background:var(--ink-1,#f3efe6);color:var(--ink-6,#3a3528);font-size:' + avFontSize + 'px;font-weight:600;border:1.5px solid var(--ink-paper,#fff);margin-left:' + (i === 0 ? '0' : '-6px') + ';">' + init + '</span>';
  });
  if (rest > 0) {
    html += '<span class="contrib-av contrib-av--more" style="display:inline-flex;align-items:center;justify-content:center;width:' + size + 'px;height:' + size + 'px;border-radius:50%;background:var(--ink-2,#e8e2d3);color:var(--ink-6,#3a3528);font-size:' + avFontSize + 'px;font-weight:600;border:1.5px solid var(--ink-paper,#fff);margin-left:-6px;">+' + rest + '</span>';
  }
  html += '</span>';
  return html;
}
...
// ── Contributor migration helpers ────────────────────────────────────
// Suggest contributors based on who's already logged time against each
// task. Conservative — only flags task+person combinations where the
// person logged at least 1 hour against a task they're not currently
// assigned to.
//
//   findContributorCandidates()  — list suggestions; no writes
//   flagContributors(map?)       — apply (after you've reviewed)
window.findContributorCandidates = function findContributorCandidates() {
  if (typeof TASKS === 'undefined' || !TASKS || typeof TIME_ENTRIES === 'undefined') {
    console.log('TASKS / TIME_ENTRIES not loaded yet.');
    return null;
  }
  // Aggregate hours per (task_idx, person) where person != assignee.
  var byTask = {}; // task_idx -> { taskRef, hoursByPerson: { name: hours } }
  var idxToTask = {};
  TASKS.forEach(function(t) { idxToTask[String(t.idx)] = t; });

  TIME_ENTRIES.forEach(function(e) {
    var t = idxToTask[String(e.task_idx)];
    if (!t) return;
    if (!e.name || e.name === t.assignee) return;
    var h = Number(e.hours_worked) || 0;
    if (h < 1) return; // ignore < 1 hour
    if (!byTask[t.idx]) byTask[t.idx] = { task: t, hoursByPerson: {} };
    byTask[t.idx].hoursByPerson[e.name] = (byTask[t.idx].hoursByPerson[e.name] || 0) + h;
  });

  var rows = [];
  Object.keys(byTask).forEach(function(idx) {
    var entry = byTask[idx];
    var existing = (typeof getTaskContributors === 'function') ? getTaskContributors(entry.task) : [];
    Object.keys(entry.hoursByPerson).forEach(function(name) {
      if (existing.indexOf(name) >= 0) return; // already a contributor
      rows.push({
        objectId: entry.task.objectId,
        task: entry.task.task_number || entry.task.idx,
        title: entry.task.title || '',
        assignee: entry.task.assignee || '(unassigned)',
        candidate: name,
        hours: Math.round(entry.hoursByPerson[name] * 10) / 10
      });
    });
  });

  if (rows.length === 0) {
    console.log('%c✓ No contributor candidates found — every time-logger is already either the assignee or a contributor.', 'color:#15803d;font-weight:700;');
    return [];
  }

  rows.sort(function(a, b) { return b.hours - a.hours; });
  console.log('%c' + rows.length + ' contributor suggestion(s) found:', 'font-weight:700;');
  console.table(rows.slice(0, 100));
  if (rows.length > 100) console.log('  …and ' + (rows.length - 100) + ' more (showing first 100; full list at window._contribCandidates)');

  // Roll up into the shape flagContributors expects: { taskObjectId: [names] }
  var byTaskMap = {};
  rows.forEach(function(r) {
    if (!byTaskMap[r.objectId]) byTaskMap[r.objectId] = [];
    byTaskMap[r.objectId].push(r.candidate);
  });
  window._contribCandidates = byTaskMap;
  console.log('%cReview the table. To apply: flagContributors() will add each candidate to their task\'s contributors list.', 'color:#6b6354;');
  return rows;
};

window.flagContributors = async function flagContributors(map) {
  map = map || window._contribCandidates;
  if (!map || Object.keys(map).length === 0) {
    console.log('No candidates. Run findContributorCandidates() first.');
    return;
  }
  if (typeof DataStore === 'undefined' || !DataStore.updateTask) {
    console.log('DataStore.updateTask not available.');
  ...
  var taskIds = Object.keys(map);
  console.log('%cAdding contributors to ' + taskIds.length + ' task(s)…', 'font-weight:700;');
  var ok = 0, fail = 0;
  for (var i = 0; i < taskIds.length; i++) {
    var objectId = taskIds[i];
    var task = (typeof DataStore.getTask === 'function') ? DataStore.getTask(parseInt(objectId, 10)) : null;
    if (!task) { fail++; continue; }
    var existing = (typeof getTaskContributors === 'function') ? getTaskContributors(task) : [];
    var merged = existing.slice();
    map[objectId].forEach(function(name) {
      if (merged.indexOf(name) < 0 && name !== task.assignee) merged.push(name);
    });
    var newValue = merged.join(', ');
    try {
      await DataStore.updateTask(parseInt(objectId, 10), { contributors: newValue });
      task.contributors = newValue;
      ok++;
    } catch (e) {
      console.warn('  failed task ' + objectId + ':', e);
      fail++;
    }
  }
  console.log('%c✓ Done — ' + ok + ' tasks updated, ' + fail + ' failed.', 'color:#15803d;font-weight:700;');
  if (typeof render === 'function') render();
};
