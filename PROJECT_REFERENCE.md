...
**URL pattern:**
```
https://services3.arcgis.com/9coHY2fvuFjG9HQX/ArcGIS/rest/services/{service}/FeatureServer/{layer-id}
```

**Local-model aliases:** The application's local data model continues to use legacy field names (e.g. `p.id`, `t.project_id`, `t.idx`, `p.start`, `t.due`) via aliases in [src/agol.js](src/agol.js); writes are translated to the new schema names (`project_number`, `task_number`, `start_date`, `due_date`, etc.) at the boundary. See `agolProjectToLocal` / `agolTaskToLocal` and the `localToAgol*` write functions for the canonical mapping.
...
|-------|------|--------|------|-------------|
| `ObjectId` | OID | — | No | Auto-generated by ArcGIS |
| `project_number` | String | 20 | No | Canonical PK (e.g. `P-549`). Aliased locally as `p.id`. |
| `title` | String | 255 | No | Project name |
| `status` | String | 50 | No | Domain: Active, Scheduled, Future, Idea, On Hold, Complete, Canceled |
| `priority` | String | 20 | Yes | Domain: High, Medium, Low |
...
| `category` | String | 100 | Yes | Domain: 13 values (project category) |
| `start_date` | Date Only | — | Yes | Planned start date. Aliased locally as `p.start`. |
| `end_date` | Date Only | — | Yes | Planned end date. Aliased locally as `p.end`. |
| `working_due` | Date Only | — | Yes | Working target end date (separate from original end_date) |
...
| `wwc_practice`, `wwc_criteria` | String | various | Yes | What Works Cities tagging |
| `deliverables` | String | 500 | Yes | Comma-separated deliverable types |
| `data_sources` | String | 500 | Yes | Free text |
| `technical_requirements` | String | 1000 | Yes | Free text |
| `leadership_title`, `leadership_summary`, `public_visibility` | various | Yes | Public-roadmap-app fields |
| `definition_of_done` | String | 4000 | Yes | Markdown-rendered acceptance criteria |
| `Creator`, `CreationDate`, `Editor`, `EditDate` | Editor tracking | — | — | Auto-populated by AGO |

**Migration notes:** Legacy fields `id` (Integer), `pid` (String), `start` (renamed → `start_date`), `end_` (renamed → `end_date`) were removed. The local-model aliases in `agolProjectToLocal` keep `p.id`, `p.start`, `p.end` working for consumer code.
...
|-------|------|--------|------|-------------|
| `ObjectId` | OID | — | No | Auto-generated by ArcGIS |
| `task_number` | String | 20 | No | Canonical PK. Aliased locally as `t.idx`. |
| `project_number` | String | 20 | No | FK to `projects.project_number`. Aliased locally as `t.project_id`. |
| `title` | String | 255 | No | Task name |
| `status` | String | 50 | No | Domain: Active, Pending, Waiting for Response, On Hold, Complete, Canceled |
| `priority` | String | 20 | Yes | Domain: High, Medium, Low |
| `assignee` | String | 100 | Yes | Assigned person |
| `category` | String | 100 | Yes | Domain: 23 values |
| `start_date` | Date Only | — | Yes | Start date. Aliased locally as `t.start`. |
| `due_date` | Date Only | — | Yes | Due date. Aliased locally as `t.due`. |
| `working_due` | Date Only | — | Yes | Working target due date |
| `actual_end` | Date Only | — | Yes | Actual completion date |
...
| `resolution` | String | 4000 | Yes | Resolution / outcome notes |
| `phase_requirements` | String | 500 | Yes | Comma-separated lifecycle requirement IDs |
| `Creator`, `CreationDate`, `Editor`, `EditDate` | Editor tracking | — | — | Auto-populated by AGO |

**Migration notes:** Legacy `idx` (Integer PK), `id` (legacy), `project_id` (Integer FK), `project` (denormalized title), `start` (renamed → `start_date`), `due` (renamed → `due_date`), `Hours_Worked` (renamed → `hours_worked`) were dropped/renamed. Aliases in `agolTaskToLocal` keep `t.idx`, `t.project_id`, `t.start`, `t.due` working.
...
| `time_tracking` | Integer | — | Yes | `1` or `0` — per-employee opt-in (was String "true"/"false" pre-2026-05) |
| `active` | Integer | — | Yes | `1` or `0` — current employee (`0` hides from rosters/dropdowns; was String pre-2026-05) |
...
| `project_number` | String(20) | No | FK to `projects.project_number` (aliased locally as `analytics_id`) |
| `project_role` | String(20) | Yes | Domain: Lead, Contributor, Reviewer |
| `week_date` | Date Only | No | Sunday start of week (YYYY-MM-DD) |
| `fraction` | Double | Yes | Allocation 0.0–1.0 of project capacity |
| `Creator`, `CreationDate`, `Editor`, `EditDate` | Editor tracking | — | Auto-populated by AGO |

**Migration notes (decisions AL1=a, AL2=a):** Dropped `project` (denormalized title), `project_status` (denormalized), `project_type` (denormalized), and `hours` (recomputed). Renamed `analytics_id` (Integer FK) → `project_number` (String FK). The allocation projection in `index.html` joins on `project_number` to look up live title/status/category from the PROJECTS array; `hours` is recomputed from `fraction × proj_cap` for the person/week.

**IMPORTANT:** `fraction` is the source of truth. `hours` is computed at projection time and is never persisted.
...
**Current keys:** `partner_depts`, `itd_teams`, `task_tools`, `allocation_defaults`, `ai_phase_assignment`, `review_types`, `productivity_ratio` (numeric string, currently `"0.9"` — used in capacity formula), `display_config`, `data_program`, `team_intro`. Status/category lists and other enumerated values formerly in app_config are now AGO domains on the relevant fields (per the 2026-05 migration).
...
**`review_types`** stores a JSON array of review-type objects used by the Project Review tab. Each entry has:

```json
{
  "id": "data-team",
  "name": "Data Team Review",
  "description": "Biweekly cross-team review…",
  "filter": { "itd_teams": ["Data Program", "Emerging Data Infrastructure - Cloud Data Services", "..."] },
  "cadence_days": 14,
  "default_attendees": ["Peter Johnson", "Adam Silva", "..."]
}
```

If the `review_types` row is missing on first load, `ensureReviewTypesSeeded()` writes the two default review types (`data-intel`, `data-team`) automatically. Edited from Settings → Project config → Review types (Team Leads only).
...
| `ObjectId` | OID | No | Auto-generated by ArcGIS |
| `name` | String(100) | No | Employee name |
| `task_number` | String(20) | No | FK to `tasks.task_number` (aliased locally as `task_idx`) |
| `project_number` | String(20) | Yes | FK to `projects.project_number` (kept for query performance per decision TE1=a; aliased locally as `project_id`) |
| `start_time` | Date | No | When work began (epoch ms) |
| `end_time` | Date | Yes | When work ended (null = timer running) |
...
**Migration notes:** Renamed `task_idx` → `task_number`, `project_id` → `project_number`. Aliases in the inline projection in `index.html` keep `e.task_idx` and `e.project_id` working for consumer code.
...
| `issue_type` | String | 20 | No | Domain: Bug, Improvement (renamed from `type_`) |
| `description` | String | 4000 | Yes | Detailed description |
| `steps_to_reproduce` | String | 2000 | Yes | How to reproduce (bugs only) |
| `status` | String | 20 | No | Domain (5 values): Submitted → Accepted → In Progress → User Testing → Done |
| `priority` | String | 10 | No | Domain: High, Medium, Low (default Medium) |
| `resolved_date` | Date Only | — | Yes | YYYY-MM-DD when marked Done |
| `Creator`, `CreationDate`, `Editor`, `EditDate` | Editor tracking | — | — | Auto-populated by AGO (replaces former `submitted_by` / `submitted_date`) |

**Migration notes:** Renamed `type_` → `issue_type`. Status domain expanded to include "User Testing" (5 values). Default values: status="Submitted", priority="Medium". Audit fields `submitted_by` / `submitted_date` moved to editor tracking; the local model exposes them as fallbacks (`a.submitted_by || a.Creator`).
...
|-------|------|--------|------|-------------|
| `ObjectId` | OID | — | No | Auto-generated by ArcGIS |
| `review_type_id` | String | 50 | No | NOT NULL. Matches an entry id in `app_config.review_types` (e.g. `data-intel`, `data-team`) |
| `project_number` | String | 20 | No | Foreign key to `projects.project_number` (e.g. `P-549`) |
| `meeting_date` | Date Only | — | No | The date the review meeting was held (sent as YYYY-MM-DD string) |
| `attendees` | String | 500 | Yes | Free-text, comma-separated names |
| `notes` | String | 4000 | Yes | Discussion notes |
| `decisions` | String | 4000 | Yes | Decisions made in the meeting |
| `action_items` | String | 4000 | Yes | Action items, typically one per line |
| `Creator`, `CreationDate`, `Editor`, `EditDate` | Editor tracking | — | — | Auto-populated by AGO (replaces former `created_by` / `created_at`) |

**Migration notes:** Field `review_id` (Integer) was dropped from the schema. The local model still carries a `review_id` for in-memory uniqueness but it isn't sent to AGO and is gradually being phased out. Audit fields `created_by` / `created_at` moved to editor tracking; the local model exposes them as fallbacks (`a.created_by || a.Creator`).

**Loaded by:** `loadProjectReviews()` (called once after `loadUserPrefs()`). Stored in the `PROJECT_REVIEWS` array.

**Permissions:** Anyone signed-in can add a review entry. Only the entry's creator (or a Team Lead) can edit/delete.
...
|-------|------|--------|------|-------------|
| `ObjectId` | OID | — | No | Auto-generated by ArcGIS |
| `note_id` | String | 36 | No | App-generated UUID (lets the local push correlate before AGO returns ObjectId) |
| `project_number` | String | 20 | No | FK to `projects.project_number` |
| `note_text` | String | 4000 | No | Note body |
| `Creator`, `CreationDate`, `Editor`, `EditDate` | Editor tracking | — | — | Auto-populated by AGO (replaces former `created_by` / `created_at`) |

**Loaded by:** `loadProjectNotes()`. Stored in the `PROJECT_NOTES` array.

**Notes are hard-deleted (no soft-delete column).**
...
|-------|------|--------|------|-------------|
| `ObjectId` | OID | — | No | Auto-generated by ArcGIS |
| `project_number` | String | 20 | No | FK to `projects.project_number` (aliased locally as `project_id`) |
| `project_title` | String | 255 | Yes | Snapshot of the project title at the time of change |
| `status` | String | 50 | No | Domain (7 values, same as projects.status) |
| `Creator`, `CreationDate`, `Editor`, `EditDate` | Editor tracking | — | — | Auto-populated by AGO (replaces former `changed_by` / `changed_date`) |

**Migration notes:** FK renamed `project_id` → `project_number`. Audit fields `changed_by` / `changed_date` moved to editor tracking; the local model normalizes `CreationDate` (epoch ms) to a `YYYY-MM-DD` string via `epochToDateStr` so consumer code reading `h.changed_date` continues to work.
...
```javascript
// Three consolidated FeatureServers (post-2026-05 migration); each hosts
// multiple tables. Layer index after FeatureServer/ identifies the table.
const ARCGIS_CONFIG = {
...
**Beta-gated:** controlled by `FEATURE_PROJECT_REVIEW = 'beta'`. Tab visibility wired via `applyBetaTabVisibility()`, called after `loadUserPrefs()` and from `updateBetaPref()` so toggling the preference takes effect immediately.

**Purpose:** Recurring portfolio reviews. The unit of review is a **project** plus the people assigned to it (via `contact` + `other_members`). Tasks on the project, when present, are shown as supporting context grouped by assignee.

**State:**
- `_currentReviewTypeId` — id of the active review type (matches an entry in `_reviewTypes`)
- `_reviewFilterMode` — `'all' | 'active' | 'due' | 'overdue' | 'has_actions'`
- `_prTasksOpen` / `_prShowAllLog` — per-project UI expand state
- `_prModalState` — current Log Review modal context

**Key functions:**
- `renderProjectReview(area)` — top-level renderer (header + cycle strip + filter chips + cards)
- `renderProjectReviewCard(project, reviewType)` — per-project card
- `prGetProjectsForReview(rt, mode)` — filters PROJECTS by `rt.filter.itd_teams` (excluding Idea status), then by mode
- `prGetRoster(project)` — derives unique people list from `contact ∪ split(other_members)`
- `prGetTasksForProject(project)` — groups TASKS by assignee
- `prCycleBadge(lastEpoch, cadenceDays)` — Fresh / Due / Overdue / Never (soft cadence enforcement: never hides projects)
- `prOpenLogModal()` / `prSaveLog()` / `prDeleteLog()` — review entry CRUD against `Project_Reviews` service
- `getLastReviewForProject(projNum, rtId)` / `getReviewsForProject(projNum, rtId)` — local helpers over `PROJECT_REVIEWS`
- `nextReviewId()` — computes `max(review_id) + 1` for new entries

**Cycle thresholds (soft):**
- `daysSince <= cadence_days` → Fresh (green)
- `daysSince <= cadence_days * 1.5` → Due (yellow)
- `daysSince > cadence_days * 1.5` → Overdue (red)
- never reviewed → dashed gray "Not yet reviewed"

**Permissions:** Anyone signed-in (not preview mode) can log a review entry. Edit/delete is restricted to the entry's `created_by` (Team Leads can also delete). Editing review *types* is Team-Lead-only via Settings → Review types.
...
proj_cap = (weekly_hours - absence_hours) × productivity_ratio × proj_pct
  where productivity_ratio = configurable factor from app_config (currently 0.9)
...
productivity_ratio = 0.9 (current app_config value)

proj_cap = (36 - 8) × 0.9 × 0.63 = 15.876 hours available

If allocated 50% to Project A: hours = 0.50 × 15.876 = 7.938
If allocated 30% to Project B: hours = 0.30 × 15.876 = 4.7628
Total allocated: 12.7008 hours
Utilization: 12.7008 / 15.876 = 80%
```
...
| 0.13.2.0002 | Preserve filters after editing a project or task (remove filter-clearing on save) |
| 0.14.0.0001 | Task notes and status change history (new capability, behind FEATURE_TASK_HISTORY flag): task_notes and task_status_history JSON fields; chronological notes thread with inline add-note UI; auto-captured status history with reason prompts for On Hold/Waiting/Canceled; merged Notes & History timeline on task detail page; hooked into mwQuickStatus, applyBatchUpdate, handleFormSubmit, and markTaskComplete |
