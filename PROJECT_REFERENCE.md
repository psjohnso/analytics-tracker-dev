...
**Current keys:** partner_depts, itd_teams, task_categories, task_tools, task_statuses, project_statuses, project_categories, allocation_defaults, ai_phase_assignment, **review_types**
...
Service URL fragment: `services/Project_Reviews/FeatureServer/0`. Table (no geometry), Create / Update / Delete enabled.

| Field | Type | Length | Null | Description |
|-------|------|--------|------|-------------|
| `OBJECTID` | OID | — | No | Auto-generated by ArcGIS |
| `review_id` | Integer | — | No | App-generated sequential id (`max(review_id) + 1`) |
| `review_type_id` | String | 50 | No | Matches an entry id in `app_config.review_types` (e.g. `data-intel`, `data-team`) |
| `project_number` | String | 20 | No | Foreign key to `projects.project_number` (e.g. `P-549`) |
| `meeting_date` | Date Only | — | No | The date the review meeting was held |
...
| `created_by` | String | 100 | Yes | `Auth.fullName` of the user who logged the entry |
| `created_at` | Date | — | Yes | Epoch ms timestamp |

**Loaded by:** `loadProjectReviews()` (called once after `loadUserPrefs()`). Stored in the `PROJECT_REVIEWS` array.

**Permissions:** Anyone signed-in can add a review entry. Only the entry's `created_by` (or a Team Lead) can edit/delete.
